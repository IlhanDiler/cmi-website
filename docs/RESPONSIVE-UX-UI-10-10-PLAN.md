...
- Abgeschlossen: Matrix-Screenshot-Refresh fuer Desktop/Mobile/Tablet im bestehenden QA-Runner nachgezogen; [tmp/visual-qa/release_qa_smoke.py](../tmp/visual-qa/release_qa_smoke.py) schreibt jetzt zusaetzlich iPad-Portrait-Screenshots pro Browser (`index-release-tablet-portrait-*`) und lief im strikten Browser-Matrix-Run (Edge, Chrome, Firefox) weiterhin mit 0 Funden durch.
